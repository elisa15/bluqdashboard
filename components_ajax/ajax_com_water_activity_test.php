...
//Edit $data to be your query 
	$sql_pagination = " SELECT 
					*
				FROM 
				default_transaction  where id != ''  AND farmer_id = '".USER_ID."'   AND type='water' ".$and_search_key." ".$and_result_type."
				ORDER BY 
					id DESC  ";
	//echo $sql_pagination;		
...
<h1>Most Recent Submissions</h1>
<div id="container2">
	<div id="container1">
    	<?php
        $sql_most_recent = "SELECT 
					*
				FROM 
					default_transaction  where id <> ''  AND farmer_id = '".USER_ID."'    AND type='water' ".$and_search_key." ".$and_result_type."
				ORDER BY 
		...
		while($row_most_recent = mysql_fetch_array($query_most_recent))
		{
		?>
		<div id="col1">
			<!-- Column one start -->
			<p><img src="<?=$row_most_recent['image']?>" width="200"/></p>
            <p><?=$row_most_recent['plant']?></p>
            <hr>
            <p><?=$row_most_recent['date_created']?></p>
			<!-- Column one end -->
		</div>
        <?php
		}
...
<table cellpadding="0" cellspacing="0" width="100%">
	...
	if($row_ctr > 0)
	{
        ...
        <tr>
            ...
            <td><a href="#" class="do_edit_action2" returnId="<?=$row['id']?>"><img src="<?=$row['image']?>" width="150px"/></a></td>
        </tr>
    <?php
		}
	} // end of IF
